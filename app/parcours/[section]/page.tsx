...
export default function SectionPage() {
  ...
  const { unitesCompleted, sectionsCompleted } = useGameStore();
  ...
  function handleUniteClick(uniteId: number) {
    if (
      !isUniteUnlocked(sectionId, uniteId, unitesCompleted, sectionsCompleted)
    )
      return;
  ...
  // Auto-complete section when all unites are done
  const { completeSection } = useGameStore();
    ...
    <div className="flex flex-col flex-1 p-6 max-w-2xl mx-auto w-full gap-6">
      ...
      <div className="flex flex-col gap-3">
        {section.unites.map((unite) => {
          const unlocked = isUniteUnlocked(
            sectionId,
            unite.id,
            unitesCompleted,
            sectionsCompleted
          );
          const completed = unitesCompleted.includes(
            `${sectionId}-${unite.id}`
          );
      ...
      {allComplete && (
        <div className="neo-border neo-shadow-lg bg-neo-teal-light p-6 text-center animate-pop-in">
          ...
          <NeoButton onClick={() => router.push("/map")}>
            Retour a la carte
          </NeoButton>
        </div>
      )}
