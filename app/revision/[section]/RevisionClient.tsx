    ...
    <div className="flex flex-col flex-1 p-6 max-w-2xl mx-auto w-full gap-6">
      ...
      {wrongQuestions.length === 0 ? (
        <div className="neo-border neo-shadow-md bg-neo-teal-light p-6 text-center">
          <p className="font-display font-extrabold text-xl mb-2">Aucune question a revoir ! 🌟</p>
          <p className="font-body text-sm">Tu as tout bon sur cette section.</p>
        </div>
      ) : (
            ...
            <NeoCard key={q.id} color="white" className="border-l-4 border-l-neo-coral">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs px-2 py-0.5 neo-border bg-gray-100">{q.type}</span>
                  <span className="font-mono text-xs text-gray-400">Unite {q.unite}</span>
                </div>
                <p className="font-heading font-bold text-sm">{q.question}</p>
